  ...
  <div id="shop" v-if='msg=="全部项目" || (categoryList && categoryList.length>0)'>
    <div class="container mat04">
      ...
      <div class="assortment">
        <ul id="quickItem">
          <li class="active showActive" v-for="(item,index) in categoryList" :key="item.id+item.name" @click="quickTwo(item.name)">
            <span>{{item.name}}</span>
          </li>
          <div class="flag"></div>
        </ul>
...
export default {
  data() {
	...
	// 初始化获取信息
	created:function(){
		sessionStorage.removeItem("category") //清空缓存里面的项目名称层级
		sessionStorage.removeItem("categoryTwo")
		sessionStorage.removeItem("categoryThree")
  ...
  methods: {
    ...
    //查询下一分类
    quickTwo(categoryName){
     this.$router.push({path:'./quickScreenTwo', query: {city:this.city,category:categoryName}}),
		 sessionStorage.setItem("category",this.msg)   //一级分类名
		 sessionStorage.setItem("categoryTwo",categoryName)  //二级分类名
    },
