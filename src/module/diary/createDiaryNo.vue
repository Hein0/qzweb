  ...
  <div id="detail">
      <div class="container">   
        <ul class="discounts">
         ...
         <router-link :to="{path:'/indent/',query:{user_id:user_id,order_status:2}}">
              <ul class="discounts">
                <li class="discount">
                  <span class="discount-text">选择订单</span>
                  <span class="time" v-if="order_sn">{{order_sn}}</span>
                </li>
              </ul> 
          </router-link> 
               <mt-button type="danger" class="Next" @click="next" :disabled="ableToNext">下一步</mt-button>   
      </div>
        <mt-datetime-picker
          ref="picker"
          type="date"
